...
class SettingsProvider with ChangeNotifier {
  ...
  Future<bool> notificationUpdateFilter(NotificationFilter filter) async {
    // Get shared preferences instance
    SharedPreferences prefs = await SharedPreferences.getInstance();
    await prefs.reload();

    // Get current filters
    final List<String> filtersJSON = prefs.getStringList(settingNLFilters) ?? <String>[];
    final List<NotificationFilter> filters = filtersJSON
        .map((String json) => NotificationFilter.fromJson(jsonDecode(json)))
        .toList();

    // Find the filter index
    final int filterIndex = filters.indexWhere((NotificationFilter e) => e.id == filter.id);

    // If filter not found, return false
    if (filterIndex == -1) {
      return false;
    }

    // Get the existing filter and update its properties
    // NotificationFilter existingFilter = filters[filterIndex];

    // Update only the provided fields
    // if (packageName != null) {
    //   existingFilter = NotificationFilter(
    //     id: existingFilter.id,
    //     packageName: packageName,
    //     pattern: existingFilter.pattern,
    //   );
    // }
    ...
    // Save the updated filters list
    await prefs.setStringList(
      settingNLFilters,
      filters.map((NotificationFilter filter) => jsonEncode(filter)).toList(),
    );

    // Update the local filters list
    _notificationFilters = filters;

    log.finest(() => "notify SettingsProvider->notificationUpdateFilter()");
    notifyListeners();
